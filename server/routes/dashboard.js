...
router.get('/summary', verifyToken, checkPermission('dashboard.view'), async (req, res) => {
    try {
        ...
        const [
            ...
            // Gross Sales
            db.query(`SELECT COALESCE(SUM(total_amount - COALESCE(credit_note_amount, 0)), 0) as total FROM sales WHERE ${dateWhereClause}`, dateQueryParam),
            // Bill Count
        ...
        const [dailySales, categories, products, atRisk, profitableItems] = await Promise.all([
            ...
            db.query(`
                SELECT 
                    ...
                    SUM(total_amount - COALESCE(credit_note_amount, 0)) as total,
                    COUNT(*) as count
                FROM sales 
            ...
            db.query(`
                SELECT 
                    ...
                    SUM(s.total_amount - COALESCE(s.credit_note_amount, 0)) as lifetime_spend
                FROM customers c
        ...
        const [repeatCustomers, topSpenders] = await Promise.all([
            db.query(`
                SELECT 
                    ...
                    SUM(s.total_amount - COALESCE(s.credit_note_amount, 0)) as total_spent
                FROM sales s
        ...
        const [hourly, payments, trending, todayStats, typicalStats, diag] = await Promise.all([
            ...
            db.query(`
                SELECT 
                    ...
                    SUM(total_amount - COALESCE(credit_note_amount, 0)) as revenue,
                    COUNT(id) as bills
                FROM sales 
            ...
            db.query(`
                SELECT 
                    ...
                    AVG(total_amount - COALESCE(credit_note_amount, 0)) as avg_checkout
                FROM sales
                WHERE created_at >= ? AND created_at < ?
            `, [startTodayStr, endTodayStr]),
            ...
            db.query(`
                SELECT AVG(total_amount - COALESCE(credit_note_amount, 0)) as typical_checkout
                FROM sales
                WHERE created_at >= ? AND created_at < ?
